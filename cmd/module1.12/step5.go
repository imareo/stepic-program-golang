...
целому беззнаковому числу. Имя массива который вы должны сами создать workArray
...
Sample Output:99 137 151 187 117 71 20 187 93 67 type ok
*/
package main
...
func main() {
	...
	var replArray [6]uint8

	for i := 0; i < len(workArray); i++ {
		_, _ = fmt.Scan(&workArray[i])
	}
	for i := 0; i < len(replArray); i++ {
		_, _ = fmt.Scan(&replArray[i])
	}
	for i := 0; i < len(replArray); i += 2 {
		idx1 := replArray[i]
		idx2 := replArray[i+1]
